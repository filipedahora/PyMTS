...
class Image:
    def __init__(
        ...
        self.stimulus_path = f"D:/PyMTS/PyMTS/src/stimuli/{stimulus_path}"  # TODO
        if stimulus_path == "n":
            return None
        self.screen = screen
        ...
        self.pos_center = (
            np.array(self.screen_size) / 2 - np.array(self.stimulus_size) / 2
        )
        self.pos_x, self.pos_y = self.pos_center + np.array(
            [stimulus_pos[0], -stimulus_pos[1]]
        )

        self.name = self.stimulus_path[-6:-4]
    ...
    def update(self):
        if self.stimulus_path == "stimuli/n":
            return None
        self.screen_size = width, height = pygame.display.get_window_size()
        self.pos_x, self.pos_y = np.array(self.screen_size) / 2 + np.array(
            [self.stimulus_pos[0], -self.stimulus_pos[1]]
        )
        self.rect = self.img.get_rect(center=(self.pos_x, self.pos_y))
    ...
    def mouse_hover(self):
        ...
        if mouse_pos[0] in range(self.rect.left, self.rect.right) and mouse_pos[
            1
